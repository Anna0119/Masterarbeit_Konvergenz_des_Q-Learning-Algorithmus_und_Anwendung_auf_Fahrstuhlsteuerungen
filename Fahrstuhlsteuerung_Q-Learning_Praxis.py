...
# Hier wird anstelle der ganzen Q-Funktion in jeder Iteration lediglich ein Q-Wert aktualisiert
...
while continue_simulation_2 == True:
    ...
    while continue_simulation == True:
        ...
        # Die Distanz von zwei Q-Funktionen wird nach 25.600 Aktualisierungen einzelner Q-Funktionen gemessen, um eine Vergleichbarkeit zum Algorithmus 1 (in der Arbeit zu gewährleisten)
        if(j%25600 == 0):
...
# Abbildung der Konvergenz des euklidischen Abstandes zwischen Q-Funktionen
y_global_delta = global_delta
x_g_d = [x for x in range(len(global_delta))]

fig = plt.figure()
plt.yscale("log")
plt.plot(x_g_d, y_global_delta, label='normierter Abstand zweier Q-Funktionen', color = 'blue', alpha = 0.5)
fig.suptitle('Veränderung zwischen zwei Q-Funktionen nach jeweils 10 Iterationen')
plt.xlabel('Abstand zweier Q-Funktionen nach 25600 einzelnen Updates')
plt.ylabel('Norm des Abstandes zwischen zwei Q-Tabellen')
plt.legend()
plt.show()
plt.close()